...
deploy_to_server() {
    ...
    if ! scp -q -o StrictHostKeyChecking=accept-new "${DEPLOY_BIN:-/tmp/xray}" "$SSH_USER@$server:/tmp/xray"; then
    ...
    if ! ssh -q -o StrictHostKeyChecking=accept-new "$SSH_USER@$server" \
        "sudo mv /tmp/xray /usr/local/bin/xray && \
         sudo systemctl restart xray"; then
